...
/*   Updated: 2025/07/24 16:54:37 by hguerrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	setup_control_components(t_control_panel *cp, t_win_config *control_data)
{
    t_button	my_button;
    t_slider	my_slider;
...
static void	display_object_image(t_control_panel *cp)
{
    ...
    if (cp->data.obj_type == 0 && data->image.sphere)
        mlx_put_image_to_window(data->mlx, data->win, data->image.sphere, 10, 10);
    else if (cp->data.obj_type == 1 && data->image.plane)
        mlx_put_image_to_window(data->mlx, data->win, data->image.plane, 10, 10);
    else if (cp->data.obj_type == 2 && data->image.cylinder)
        mlx_put_image_to_window(data->mlx, data->win, data->image.cylinder, 10, 10);
...
int	create_control_window(t_control_panel *cp)
{
    ...
    mlx_string_put(control_data->mlx, control_data->win, 175, 130,
        0xFF0000, "RENDER");
