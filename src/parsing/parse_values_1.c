...
/*   Updated: 2025/10/12 17:52:43 by hguerrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_error_log *error_log)
{
	...
	if (!info || (len == ft_strlen("LAMBERTIAN") && !ft_strncmp(info,
				"LAMBERTIAN", len)))
		set_lambertian_material(object_material, rgb);
	else if (len == ft_strlen("METAL") && !ft_strncmp(info, "METAL",
			len))
		set_metal_material(object_material, rgb);
	else if (len == ft_strlen("GLASS") && !ft_strncmp(info, "GLASS", len))
		set_glass_material(object_material);
	else if (len == ft_strlen("CHECKERPATTERN") && !ft_strncmp(info,
			"CHECKERPATTERN", len))
		set_checker_material(object_material, rgb);
	else if (len == ft_strlen("SOLID") && !ft_strncmp(info, "SOLID", len))
