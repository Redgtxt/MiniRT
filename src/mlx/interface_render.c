...
static void	draw_interface_labels(t_control_panel *cp)
{
	if (!cp || !cp->config_win)
		return ;
	mlx_string_put(cp->config_win->mlx, cp->config_win->win, 235, 15, 0xFFFFFF,
		"=== MiniRT Control Panel ===");
	mlx_string_put(cp->config_win->mlx, cp->config_win->win, 50, 185, 0xFFFFFF,
		"=== Ambient Light: ===");
	mlx_string_put(cp->config_win->mlx, cp->config_win->win, 50, 360, 0xFFFFFF,
		"=== Material: ===");
	mlx_string_put(cp->config_win->mlx, cp->config_win->win, 175, 130, 0xFF4040,
		"RENDER");
	mlx_string_put(cp->config_win->mlx, cp->config_win->win, 50, 235, 0xFFFFFF,
		"== Object Colors ==");
}
