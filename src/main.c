...
/*   Updated: 2025/06/09 18:07:43 by hguerrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char *argv[])
{
    ...
    while (y < control_panel->camera.image_height)
	{
        ...
        while (x < control_panel->camera.image_width)
		{
			...
			vec3_scale(pixel_h, control_panel->camera.pixel_delta_u, x);
			vec3_scale(pixel_v, control_panel->camera.pixel_delta_v, y);
			vec3_add(sum_pixel, pixel_h, pixel_v);
			// Estou a andar para a  posicao de onde o raio vai sair
			vec3_add(pixel_center, control_panel->camera.pixel00_loc, sum_pixel);
