...
void	print_elements(t_control_panel *control_panel)
{
	...
	printf("\t rgb = r-> %f g-> %f b-> %f\n", control_panel->amb_light.rgb[0],
		control_panel->amb_light.rgb[1], control_panel->amb_light.rgb[2]);
	printf("\nCamera:\n");
	printf("\t coord = x-> %.2f y-> %.2f z-> %.2f\n",
		control_panel->camera.cords[0], control_panel->camera.cords[1],
		control_panel->camera.cords[2]);
	printf("\t vector = x-> %.2f y-> %.2f z-> %.2f\n",
		control_panel->camera.vec3[0], control_panel->camera.vec3[1],
		control_panel->camera.vec3[2]);
	...
	if (control_panel->cone && control_panel->data.cone_count > 0)
	{
		...
		for (size_t i = 0; i < control_panel->data.cone_count; i++)
		{
			printf("\nCone %zu:\n", i + 1);
			printf("\t coord = x-> %.2f y-> %.2f z-> %.2f\n",
				control_panel->cone[i].cords[0],
				control_panel->cone[i].cords[1],
				control_panel->cone[i].cords[2]);
			printf("\t vector = x-> %.2f y-> %.2f z-> %.2f\n",
				control_panel->cone[i].vec3[0], control_panel->cone[i].vec3[1],
				control_panel->cone[i].vec3[2]);
			printf("\t d = %.2f\n", control_panel->cone[i].d);
			printf("\t radius = %.2f\n", control_panel->cone[i].radius);
			printf("\t height = %.2f\n", control_panel->cone[i].height);
			printf("\t rgb = r-> %f g-> %f b-> %f\n",
				control_panel->cone[i].rgb[0], control_panel->cone[i].rgb[1],
				control_panel->cone[i].rgb[2]);
		}
	}
	else
	{
