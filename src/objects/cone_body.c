...
/*   Updated: 2025/09/30 18:30:00 by ruigoncalve      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_cone_body_vars(t_cone_body_vars *vars, t_cone *cone,
						const t_ray *ray)
{
	vec3_add_scaled(vars->cone_tip, cone->cords, cone->vec3,
							cone->height / 2.0);
	...
	vars->a = vec3_dot(ray->direction, ray->direction) -
							(1 + vars->k) * vars->dot_d_v * vars->dot_d_v;
	vars->b = 2 * (vec3_dot(ray->direction, vars->oc) -
							(1 + vars->k) * vars->dot_d_v * vars->dot_oc_v);
	vars->c = vec3_dot(vars->oc, vars->oc) -
							(1 + vars->k) * vars->dot_oc_v * vars->dot_oc_v;
}
						...
						t_interval t_ray)
{
	...
	if (interval_contains(vars->t1, t_ray) && vars->m1 < 0
					&& vars->m1 > -cone->height)
		vars->t = vars->t1;
	vars->m2 = vars->dot_d_v * vars->t2 + vars->dot_oc_v;
	if (interval_contains(vars->t2, t_ray) && vars->m2 < 0
					&& vars->m2 > -cone->height)
	{
