...
/*   Updated: 2025/09/30 18:33:28 by ruigoncalve      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					const t_ray *ray, t_interval t_ray, t_hit_record *record)
{
    ...
    if (have_hit_cylinder_cap(vars->cylinder, ray,
			interval_create(t_ray.min, vars->closest_so_far), &vars->temp_rec))
    {
					...
					const t_ray *ray, t_interval t_ray, t_hit_record *record)
{
    *vars->cylinder = vars->original_cylinder;
    if (hit_cylinder_body(vars->cylinder, ray,
			interval_create(t_ray.min, vars->closest_so_far), &vars->temp_rec))
    {
        vars->hit_anything = true;
        vars->closest_so_far = vars->temp_rec.t;
